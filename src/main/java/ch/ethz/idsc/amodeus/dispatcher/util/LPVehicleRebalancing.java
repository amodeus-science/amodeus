...
public class LPVehicleRebalancing {
            ...
            // 1) VARIABLES and constraints on variables (non-negativity, fixed to zero)
            {
                ...
                for (int i = 0; i < n; ++i) {
                    for (int j = 0; j < n; ++j) {
                        ...
                        Optional<VirtualLink<Link>> optVLink = virtualNetwork.getVirtualLinks().stream().filter(v -> (v.getFrom().equals(vNodeFrom) && v.getTo().equals(vNodeTo)))
                                .findFirst();
    ...
    // function is never called! will result in memory leaks
    public void closeLP() {
