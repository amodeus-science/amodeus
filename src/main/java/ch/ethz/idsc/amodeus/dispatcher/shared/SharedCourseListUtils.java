...
public enum SharedCourseListUtils {
    ...
    public static Set<AVRequest> getUniqueAVRequests(List<? extends SharedCourse> courses) {
        return courses.stream().filter(sc -> !sc.getMealType().equals(SharedMealType.REDIRECT) && !sc.getMealType().equals(SharedMealType.WAIT)).map(sc -> sc.getAvRequest()).collect(Collectors.toSet());//
    }
