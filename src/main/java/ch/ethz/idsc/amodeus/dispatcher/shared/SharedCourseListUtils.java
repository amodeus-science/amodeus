...
/**
 * This class gives some functionalities on modifiable Lists of Shared AV
 * Courses. It extends normal List functionalities with some Tests
 */
public enum SharedCourseListUtils {
	...
	private static Set<AVRequest> getRequestsWithMealType(List<? extends SharedCourse> courses,
			SharedMealType sharedMealType) {
		return courses.stream().filter(sc -> sc.getMealType().equals(sharedMealType)).map(sc -> sc.getAvRequest())
	...
	public static long getNumberWaits(List<? extends SharedCourse> courses) {
		return getNumberSharedMealType(courses, SharedMealType.WAIT);
	}

	public static long getNumberParks(List<? extends SharedCourse> courses) {
		return getNumberSharedMealType(courses, SharedMealType.PARK);
	}
	...
	public static Set<AVRequest> getUniqueAVRequests(List<? extends SharedCourse> courses) {
		return courses.stream().filter(sc -> !sc.getMealType().equals(SharedMealType.REDIRECT)
	...
	/**
	 * @return false if any dropoff occurs after pickup in the menu. If no dropoff
	 *         ocurs for one pickup an exception is thrown as this is not allowed in
	 *         a {@link SharedMenu}
	 */
		...
		for (SharedCourse course : courses) {
			if (course.getMealType().equals(SharedMealType.PICKUP)) {
				...
				if (pickupIndex > dropofIndex) {
					System.err
							.println("The SharedRoboTaxiMenu contains a pickup after its dropoff. Stopping Execution.");
					return false;
				}
			}
		}
	...
	public static boolean checkMenuDoesNotPlanToPickUpMoreCustomersThanCapacity(List<? extends SharedCourse> courses,
			int roboTaxiCapacity) {
		...
		for (SharedCourse sharedAVCourse : courses) {
			if (sharedAVCourse.getMealType().equals(SharedMealType.PICKUP)) {
				...
				// --
			} else if (sharedAVCourse.getMealType().equals(SharedMealType.WAIT)) {
				// --
			} else if (sharedAVCourse.getMealType().equals(SharedMealType.PARK)) {
				// --
			} else {
