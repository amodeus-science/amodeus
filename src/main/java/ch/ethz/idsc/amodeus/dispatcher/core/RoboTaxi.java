...
public class RoboTaxi {
	...
	// TODO Added by Carl
	/**get the status if drop off is in progress*/
	public boolean isDropoffInProgress() {
	...
	/* package */ boolean notDrivingOnLastLink() {
		...
		if (avT instanceof AVStayTask) {
			...
			if (!usageType.equals(RoboTaxiUsageType.SHARED)) { // for shared this is allowed e.g. when a new course is
																// added but the it has not been executed yet
				logger.warn(
						"RoboTaxiStatus != STAY, but Schedule.getCurrentTask() == AVStayTask; probably needs fixing");
	...
	private void updateMenu(SharedMenu menu) {
		...
		if (!SharedCourseListUtils.checkMenuConsistency(getUnmodifiableViewOfCourses(), getCapacity())) {
	...
	/* package */ void removeAVRequestFromMenu(AVRequest avRequest) {
		...
		SharedCourse dropoffCourse = SharedCourse.dropoffCourse(avRequest);
		GlobalAssert
				.that(menu.getRoboTaxiMenu().contains(pickupCourse) && menu.getRoboTaxiMenu().contains(dropoffCourse));
