...
public class AmodeusVirtualNetworkModeModule extends AbstractDvrpModeModule {
    private final static Logger logger = Logger.getLogger(AmodeusVirtualNetworkModeModule.class);

    private final AmodeusModeConfig modeConfig;

    public AmodeusVirtualNetworkModeModule(AmodeusModeConfig modeConfig) {
    ...
    public void installVirtualNetwork() {
        ...
        if (virtualNetworkPath == null) {
            ...
            }).toProvider(modalProvider(AmodeusVirtualNetworkModeModule::provideVirtualNetworkFromScenarioOptions)).in(Singleton.class);
        } else {
            bindModal(new TypeLiteral<VirtualNetwork<Link>>() {
            }).toProvider(modalProvider(AmodeusVirtualNetworkModeModule::provideVirtualNetworkFromConfig)).in(Singleton.class);
        }
    }
    ...
    public void installTravelData() {
        ...
        if (travelDataPath == null) {
            ...
            bindModal(TravelData.class).toProvider(modalProvider(AmodeusVirtualNetworkModeModule::provideTravelDataFromScenarioOptions)).in(Singleton.class);
        } else {
            bindModal(TravelData.class).toProvider(modalProvider(AmodeusVirtualNetworkModeModule::provideTravelDataFromConfig)).in(Singleton.class);
        }
    }
}
