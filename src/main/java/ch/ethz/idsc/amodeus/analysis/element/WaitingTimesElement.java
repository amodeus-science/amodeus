...
public class WaitingTimesElement implements AnalysisElement, TotalValueAppender {
	...
	public static final String[] WAITTIMES_LABELS = new String[] { "10% quantile", "50% quantile", "95% quantile",
			"Mean" };
	...
	public void register(SimulationObject simulationObject) {
		...
		Tensor submission = Tensor.of(waitingRequests(simulationObject.requests)
				.map(rc -> RealScalar.of(simulationObject.now - rc.submissionTime)));
		waitingRequests(simulationObject.requests)
				.forEach(rc -> requestWaitTimes.put(rc.requestIndex, simulationObject.now - rc.submissionTime));

		waitTimePlotValues.append(Join.of(StaticHelper.quantiles(submission, PARAM),
	...
	public Map<TotalValueIdentifier, String> getTotalValues() {
		...
		totalValues.put(TotalValueIdentifiersAmodeus.WAITTIMEQUANTILE1,
				String.valueOf(totalWaitTimeQuantile.Get(0).number().doubleValue()));
		totalValues.put(TotalValueIdentifiersAmodeus.WAITTIMEQUANTILE2,
				String.valueOf(totalWaitTimeQuantile.Get(1).number().doubleValue()));
		totalValues.put(TotalValueIdentifiersAmodeus.WAITTIMEQUANTILE3,
