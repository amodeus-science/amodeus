...
public enum FleetEfficiencyHtml implements HtmlReportElement {
	...
	public Map<String, HtmlBodyElement> process(AnalysisSummary analysisSummary) {
		...
		Tensor total = notWithCustomer.stream().reduce(Tensor::add).get();
		Tensor totalFiltered = StaticHelper.FILTER_ON
				? StaticHelper.filtered(Transpose.of(Tensors.of(total)), StaticHelper.FILTERSIZE)
				: Transpose.of(Tensors.of(total));	
				
		double dataPointTotal;
		double maxE = 0;
		for (int j = 0; j < total.length(); ++j) {
			dataPointTotal = totalFiltered.get(j).Get(0).number().doubleValue();
			if (maxE < dataPointTotal) {
				maxE = dataPointTotal;
			}
		}
		
		double maxEmptyRoboTaxi = Math.round(maxE);

		Tensor waitingVehicles = statusesTensor.get(RoboTaxiStatus.WAITING.ordinal());
		Tensor waitingVehiclesFiltered = StaticHelper.FILTER_ON
				? StaticHelper.filtered(Transpose.of(Tensors.of(waitingVehicles)), StaticHelper.FILTERSIZE)
				: Transpose.of(Tensors.of(waitingVehicles));
		...
		fEElement.getHTMLGenerator().insertImg(IMAGE_FOLDER + "/" + DistanceDistributionOverDayImage.FILENAME + ".png",
				800, 600);
		fEElement.getHTMLGenerator().insertImg(IMAGE_FOLDER + "/" + OccupancyDistanceRatiosImage.FILENAME + ".png", 800,
				600);
		fEElement.getHTMLGenerator().insertImg(IMAGE_FOLDER + "/" + NumberPassengerStatusDistribution.FILENAME + ".png",
				800, 600);
		fEElement.getHTMLGenerator().insertImg(IMAGE_FOLDER + "/" + EmptyRoboTaxiStatusDistribution.FILENAME + ".png",
				800, 600);
		fEElement.getHTMLGenerator().insertImg(IMAGE_FOLDER + "/" + NumberWaitingRoboTaxi.FILENAME + ".png", 800, 600);
		fEElement.getHTMLGenerator().insertImg(
				IMAGE_FOLDER + "/" + RideSharingDistributionCompositionStack.FILENAME + ".png", CompositionStack.WIDTH,
				CompositionStack.HEIGHT);
