...
public class VehicleStatistic {
	...
	public final Tensor distanceParking;
	...
	public VehicleStatistic(int tics_max, MatsimAmodeusDatabase db) {
		...
		distanceParking = Array.zeros(tics_max);
	}
	...
	/**
	 * this function is called when the {@link RoboTaxi} has changed the link, then
	 * we can register the distance covered by the vehicle on the previous link and
	 * associate it to timesteps. The logic is that the distance is added evenly to
	 * the time steps.
	 */
	public void consolidate() {
		if (!list.isEmpty()) {
			...
			/**
			 * this total distance on the link was travelled on during all simulationObjects
			 * stored in the list.
			 */
			...
			for (VehicleContainer vehicleContainer : list) {
				...
				if (index < distanceTotal.length()) {
					...
					case REBALANCEDRIVE:
						...
						break;
					case PARKING:
						distanceParking.set(stepDistcontrib, index);
						distanceTotal.set(stepDistcontrib, index); // applies to all three
						break;
					default:
						break;
	...
	/**
	 * @return latest recording of Tensor {distanceTotal,
	 *         distanceWithCustomer,distancePickup,distanceRebalancd}
	 */
	public Tensor getLatestRecordings() {
		return Tensors.of( //
				...
				distanceParking.Get(lastUpdatedDist));
	}
