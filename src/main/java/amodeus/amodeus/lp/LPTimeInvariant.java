	...
	private void initRowDeltaV_i(SWIGTYPE_p_int ind, SWIGTYPE_p_double val, int timeIndex) {
		...
		for (int i = 0; i < nvNodes; i++) {
			...
			for (int j = 0; j < nvNodes; j++) {
				if (i == j)
					continue;
				bound += ((Scalar) lambdaAbsolute_ij.get(timeIndex, j, i)).number().doubleValue()
						- ((Scalar) lambdaAbsolute_ij.get(timeIndex, i, j)).number().doubleValue();
			}
			// set name
	...
	private void readAlpha_ij(int timeIndex) {
		...
		for (int i = 0; i < nvNodes; i++)
			for (int j = 0; j < nvNodes; j++) {
				if (i == j)
					continue;
				alphaAbsolute.set(RealScalar.of(GLPK.glp_get_col_prim(lp, alphaIDvarID.get(Arrays.asList(i, j)))), i,
						j);
			}
