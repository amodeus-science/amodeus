...
public class LinkLayer extends ViewerLayer {
	...
	protected void paint(Graphics2D graphics, SimulationObject ref) {
		if (drawLinks) {
			...
			NdMap<Street> map = new NdTreeMap<>(Array.zeros(2), Tensors.vector(dimension.width, dimension.height), 12,
					8);
			// draw links
	...
	private void drawLastCoord(Graphics2D graphics) {
		...
		if (Objects.nonNull(p)) {
			...
			graphics.drawString(String.format("[lng=%.6f, lat=%.6f]", lastCoord.getLon(), lastCoord.getLat()), p.x + 10,
					p.y - 10);
		}
	}
